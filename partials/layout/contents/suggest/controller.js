...
define(['projectSugar'], function () {
  ...
  suggestModule.controller('suggestController', ['$scope', '$http','$window', '$ocLazyLoad', 'suggestService', 'layoutService',
    function($scope, $http,$window, $ocLazyLoad, suggestService, layoutService) {
      ...
      $ocLazyLoad
        ...
        .then(function() {
          ...
          if($mustHave.is(':visible')) $mustHaveOn.addClass('on');
          if($mightBe.is('.on')) $mightBeOn.addClass('on');
          ...
          var sidx = $.cookie("sidx");
            ...
            //스코크 변수가 있으면 스코프 변수를 이용

            //쿠키 스코프 변수 모두 없을 경우 로그인 회원 정보로 디비에서 값을 가져온다.
          ...
          if(sidx != undefined && sidx != "") {
              ...
              $http.get(apiServer+'/insurance/sugarInfo',{params:{sidx:sidx}})
                  .success(function(data, status, headers, config) {
                      ...
                      $scope.userAge = getInsurAge(data.memInfo.birthday);
                      $scope.userSex = data.memInfo.sex;
                          ...
                          .success(function (data, status, headers, config) {
                              ...
                              if(child == "유") {
                                  //자녀 인원수 만큼 구하기
                                      ...
                                      //자녀 정보 새로 가져오기. (재귀 함수 이용)
                                      getChildInsu($http, $scope,childList,data.medicalMoney, function() {  });
...
//자녀 실비 정보 가져오기
function getChildInsu($http, $scope,childInfo, insurpay, callback) {


    var childAge = getInsurAge(childInfo[$scope.medicalChildInfoList.length].age);
    var childSex = childInfo[$scope.medicalChildInfoList.length].sex=="남아"?"M":"F";
     var cSex = childInfo[$scope.medicalChildInfoList.length].sex;

    $http.get(apiServer+'/insurance/health', {params: {insurage: childAge, insurPay: insurpay, insurType:'medical', sex:childSex}})
        .success(function(data) {


            $scope.medicalChildProductList.push(data);
            $scope.medicalChildInfoList.push({sex:cSex, age:childAge});

            if(childInfo.length > $scope.medicalChildInfoList.length) getChildInsu($http, $scope,childInfo,insurpay, callback);
            else callback();
