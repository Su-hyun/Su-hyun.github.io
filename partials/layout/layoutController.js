...
define(
  ...
  function() {
      ...
      $scope.$on('$includeContentLoaded',function(event, file) {
        if (file === 'partials/layout/header/') {
          ...
          $ocLazyLoad.load([
              ...
              'partials/layout/contents/main/js/main.js'
          ]);
          layoutService.fire();
    ...
    // 메인 페이지 콘트롤러 추가
    // 슬라이드
    layoutIndexModule.controller('eventCtrl', function ($scope) {
      $scope.eventList = [
        {
    ...
    //슈가의 보험 추천이 일리있는 이유
    layoutIndexModule.controller('resonCtrl', function ($scope) {
    ...
    // 보험의 진실편
    layoutIndexModule.controller('truthCtrl', function ($scope) {
      $scope.truthList = [
        {
